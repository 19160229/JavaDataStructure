...
//    }
    ...
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        Arrays.sort(candidates);
        getCombinationSum(candidates,target,resLists,res,0);
        return resLists;
    }

    private void getCombinationSum(int[] candidates,int target,List<List<Integer>> resLists,
                                   List<Integer> res,int index){
        if (target==0){
            resLists.add(res);
            return;
        }
        ...
        for (int i=index;i<candidates.length&&candidates[i]<=target;i++){
            ...
            getCombinationSum(candidates,target-candidates[i],resLists,tmpRes,i);
        }
    }
